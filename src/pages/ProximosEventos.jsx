  ...
  const fetchEventos = async () => {
    try {
      ...
      const fetchedEventos = response.data.map(ev => ({
        ...
        fecha: ev.fecha_evento ? new Date(ev.fecha_evento).toISOString().split('T')[0] : 'Sin fecha',
        hora: ev.horainicio_evento ? String(ev.horainicio_evento).substring(0, 5) : '',
        horaFin: ev.horafin_evento ? String(ev.horafin_evento).substring(0, 5) : '',
